...
import { NoteDataService, NoteDynamoDbDataService, DynamoDBClientOptions } from '@notes-app/database-service'
import { NoteObjectService, NoteS3ObjectService } from '@notes-app/storage-service'
import { NoteQueueService, NoteSQSQueueService, SQSClientOptions } from '@notes-app/queue-service'
...
export function installNoteDataService(): RequestHandler {
    return (req: Request, res: Response, next: NextFunction) => {
        const { noteObjectService: objectService, noteQueueService: queueService } = req
        const options: DynamoDBClientOptions = {
            objectService, queueService
        }
